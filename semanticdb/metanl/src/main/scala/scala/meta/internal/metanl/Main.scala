...
class Main(settings: Settings, reporter: Reporter) {
  ...
  private class SymbolVisitor(symsMap: Map[String, s.SymbolInformation], declPositions: Map[String, s.Range]) {
    ...
    def visit(info: s.SymbolInformation, visitingPosition: Boolean = false): Unit = {
      if (visitingPosition || (!symbolOrder.contains(info.symbol) && !declPositions.contains(info.symbol))) {
  ...
  private def transform(doc: s.TextDocument): s.TextDocument = {
    ...
    val symsMap = doc.symbols.map { info => info.symbol -> info }.toMap

    val declPositions = doc.occurrences.filter(_.role == s.SymbolOccurrence.Role.DEFINITION).map {
      occ => occ.symbol -> occ.range.get
    }.toMap
...
//    println(doc.toProtoString)
    ...
    val infosWithPositions = doc.symbols.collect {
  ...
  implicit val rangeOrdering: Ordering[s.Range] = Ordering.by(r =>
